...
* ubuntu-18.04-server 64bit
   * 工作主机：koneath@ubuntu16，IP：192.168.56.101
   * 目标主机：u2@ubuntu，IP：192.168.56.102
...
* **配置工作主机SSH免密登录目标主机**
  ...
  * root免密登录：`ssh  u2@192.168.56.102`
   ![免密登陆结果展示](/images/1.png)

* **FTP**
  * 服务器选择：vsftpd
  * 脚本文件：[vsftpd.sh](/shell/dhcp.sh)
  * 配置文件：[/etc/vsftpd.conf](/config/vsftp.conf)
  * 通过工作主机运行脚本在目标主机安装vsftpd并完成相关配置

    * 将脚本文件`vsftpd.sh`拷贝到目标主机

    * 借助SSH远程登陆目标主机，执行
    ![运行脚本结果展示](/images/2.png)
  ...
  * 配置一个支持用户名和密码方式访问的账号，该账号继承匿名访问者所有权限，且拥有对另1个独立目录及其子目录完整读写（包括创建目录、修改文件、删除文件等）权限
    ...
    * 在koneath用户的目录下出现了匿名用户目录下的文件，可证明继承匿名者访问权限
     ![继承权限](/images/3.png)
...
* **NFS**
  * 脚本文件
    * [nfs_s.sh]([/shell/nfs_s.sh))
    * [nfs_c.sh](/shell/nfs_c.sh))
  * 配置文件
    * [exports.conf](/config/exports)
  * 将nfs_s.sh拷贝到目标主机，工作主机运行nfs_s.sh脚本
   `scp -i ~/.ssh/id_rsa -r /home/koneath/nfs_s.sh u2@192.168.56.102:workspace/nfs_s.sh`、
  ...
  * 客户端共享目录中文件、子目录的属主、权限信息和在NFS服务器端的信息,uid和gid一致
    ...
    * server
     ...
     * 对于设置了no_root_squash的共享目录及对于没有设置no_root_squash的共享目录，无法在工作主机(client)写入文件
      



* **DHCP**
  * 脚本文件 [dhcp.sh](/shell/dhcp.sh)
  * 配置文件 
...
* **Samba**
  ...
  * 参考
    * [linux-public-JuliBeacon](https://github.com/CUCCS/2015-linux-public-JuliBeacon/blob/exp6/%E5%AE%9E%E9%AA%8C%206/%E5%9C%A8Linux%E5%92%8CWindows%E9%97%B4%E8%AE%BE%E7%BD%AE%E6%96%87%E4%BB%B6%E5%85%B1%E4%BA%AB.md)
